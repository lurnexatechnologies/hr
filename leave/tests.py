...
class LeavePolicyTests(TestCase):
    ...
    @patch('core.utils.get_local_date')
    def test_get_initial_leave_balance_prior_year(self, mock_get_date):
        employee = {
            'EmploymentType': 'Permanent',
            'JoinedDate': '2025-05-15'
        }
        mock_get_date.return_value = datetime.date(2026, 6, 1)
        self.assertEqual(get_initial_leave_balance(employee, 'SL'), 12.0)
        self.assertEqual(get_initial_leave_balance(employee, 'CL'), 12.0)

    @patch('core.utils.get_local_date')
    def test_get_initial_leave_balance_current_year_may(self, mock_get_date):
        employee = {
            'EmploymentType': 'Permanent',
            'JoinedDate': '2026-05-15'
        }
    ...
    @patch('core.utils.get_local_date')
    def test_get_initial_leave_balance_current_year_december(self, mock_get_date):
        employee = {
            'EmploymentType': 'Permanent',
            'JoinedDate': '2026-12-01'
        }
        mock_get_date.return_value = datetime.date(2026, 12, 10)
    ...
    @patch('core.utils.get_local_date')
    def test_refresh_monthly_leaves_normal_month(self, mock_get_date, mock_summary, mock_table):
        ...
        mock_get_date.return_value = datetime.date(2026, 5, 1)
    ...
    @patch('payroll.views.get_attendance_summary')
    @patch('core.utils.get_local_date')
    def test_refresh_monthly_leaves_jan_first(self, mock_get_date, mock_summary, mock_table):
        mock_summary.return_value = {'paid_days': 20.0}
        employee = {
            'EmployeeID': 'EMP001',
            'EmploymentType': 'Permanent',
        ...
        mock_get_date.return_value = datetime.date(2026, 1, 1)
        refresh_monthly_leaves(employee)
    ...
    @patch('core.utils.get_local_date')
    def test_get_initial_leave_balance_with_fulltime_date(self, mock_get_date):
        employee = {
            'EmploymentType': 'Permanent',
            'JoinedDate': '2026-01-01',
            'FullTimeDate': '2026-05-15'
        }
        mock_get_date.return_value = datetime.date(2026, 6, 1)
        # May is month 5: 12 - 5 + 1 = 8.0
        self.assertEqual(get_initial_leave_balance(employee, 'SL'), 8.0)
    ...
    @patch('payroll.views.get_attendance_summary')
    @patch('core.utils.get_local_date')
    def test_refresh_monthly_leaves_before_fulltime_date(self, mock_get_date, mock_summary, mock_table):
        mock_summary.return_value = {'paid_days': 20.0}
        employee = {
            'EmployeeID': 'EMP001',
            'EmploymentType': 'Permanent',
            'JoinedDate': '2026-01-01',
            'FullTimeDate': '2026-06-15',
            'LastLeaveRefresh': '2026-04',
            'Balance_PL': '5.0'
        }
        # Refreshed on May 1st - before FullTimeDate of June 15th
        mock_get_date.return_value = datetime.date(2026, 5, 1)
